...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        ...
        let allowed_statuses: Vec<String>= vec!("To-Do".to_string(), "In Progress".to_string(), "Done".to_string());
        if !allowed_statuses.contains(&status) {panic!("Only `To-Do`, `In Progress`, and `Done` statuses are allowed")}
